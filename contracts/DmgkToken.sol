...
contract DarkMAGICK is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DarkMAGICKDividendTracker public dividendTracker;
    ...
    // address public _pinkLockAddress = 0x7ee058420e5937496F5a2096f04caA7721cF70cc; // Pinksale token locker address
    address public _marketingWalletAddress = 0x1Ca43437430d1845F5Ff43bf73d36568d00740C7;
    address public _pinkLockAddress = 0x574eC5a661c3eaA610438C4a249BCFC066c4a579; // Pinksale token locker address
    uint256 public sellFeeIncreaseFactor = 100;
    uint256 public maxCapSellFeeIncreaseFactor = 300;
    ...
    event buyFeesRemovedUpdated(bool enabled);
    ...
    //   IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);// 0x10ED43C718714eb63d5aA57B78B54704E256024E; Mainnet
    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);// 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
        ...
        dividendTracker.excludeFromDividends(_pinkLockAddress);
        ...
        excludeFromFees(_pinkLockAddress, true);

        // exclude from max tx
        _isExcludedFromMaxTx[owner()] = true;
        _isExcludedFromMaxTx[address(this)] = true;
        _isExcludedFromMaxTx[_marketingWalletAddress] = true;
        _isExcludedFromMaxTx[_pinkLockAddress] = true;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DarkMAGICK: The dividend tracker already has that address");

        DarkMAGICKDividendTracker newDividendTracker = DarkMAGICKDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DarkMAGICK: The new dividend tracker must be owned by the DarkMAGICK token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DarkMAGICK: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DarkMAGICK: Account is already the value of 'excluded'");
    ...
    function updatebuyFeesRemoved(bool _enabled) public onlyOwner {
        buyFeesRemoved = _enabled;
        emit buyFeesRemovedUpdated(_enabled);
    }
    ...
    function setSellTransactionMultiplier(uint256 _multiplier) external onlyOwner {
        if(_multiplier > maxCapSellFeeIncreaseFactor)
          sellFeeIncreaseFactor = maxCapSellFeeIncreaseFactor;
        else
    ...
    function setBUSDRewardsFee(uint256 value) external onlyOwner{
        ...
        else if(value > maxCapBUSDRewardsFee)
            BUSDRewardsFee = maxCapBUSDRewardsFee;
        else
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DarkMAGICK: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DarkMAGICK: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "DarkMAGICK: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DarkMAGICK: Cannot update gasForProcessing to same value");
  ...
  function processDividendTracker(uint256 gas) external {
    ...
    uint256 contractTokenBalance = balanceOf(address(this));
        ...
        if(buyFeesRemoved == true && automatedMarketMakerPairs[from]) {
          takeFee = false;
        }
        ...
        if(takeFee) {
          ...
          // if sell, multiply by set sellFeeIncreaseFactor
...
contract DarkMAGICKDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("DarkMAGICK_Dividen_Tracker", "DarkMAGICK_Dividend_Tracker") {
      claimWait = 3600;
        minimumTokenBalanceForDividends = 50000 * (10**18); //must hold 50000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "DarkMAGICK_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "DarkMAGICK_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DarkMAGICK contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DarkMAGICK_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DarkMAGICK_Dividend_Tracker: Cannot update claimWait to same value");
